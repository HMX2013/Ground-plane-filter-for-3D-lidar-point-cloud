...
#include <ground_plane_fitting/ground_plane_fitting_Config.h>
...
private:
    ...
    dynamic_reconfigure::Server<ground_plane_fitting::ground_plane_fitting_Config> server;
    dynamic_reconfigure::Server<ground_plane_fitting::ground_plane_fitting_Config>::CallbackType f;


    void GPF_algorithm_core(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg);
...
void dynamicParamCallback(ground_plane_fitting::ground_plane_fitting_Config& config, uint32_t level)
{
  // ground_plane_fitting Parameters
    sensor_height_=config.sensor_height;
    th_dist_=config.threshold_distance_from_the_plane;
    num_iter_=config.number_of_iterations;
    num_lpr_=config.number_point_to_estimate_lpr;
    th_seeds_=config.threshold_for_initial_seeds;
...
GroundPlaneFit::GroundPlaneFit():node_handle_("~"){
    ...
    points_node_sub_ = node_handle_.subscribe(point_topic_, 2, &GroundPlaneFit::GPF_algorithm_core, this);
...
*/
void GroundPlaneFit::GPF_algorithm_core(const sensor_msgs::PointCloud2ConstPtr& in_cloud_msg){
